...
Talos は `VolumeConfig` の `encryption` で STATE と EPHEMERAL に LUKS2 を掛けられる。
R9 の監査で候補に挙がったが、入れないことにした。

鍵の種類は4つある（`nodeID`、`static`、`kms`、`tpm`）。
追加の鍵管理が要らないのは `nodeID` で、ノード UUID とパーティションラベルから導出する。

**`nodeID` が守るのは「ディスクだけ抜かれたとき」に限られる。**
鍵はノードから導出されるため、機体ごと持って行かれれば復号される。
さらに STATE の暗号化設定は META パーティションに平文で保存される。

強くするには `tpm` 鍵と SecureBoot が要る。
そこまで積むと、ノードの作り直しやマザーボード交換のたびに鍵の扱いが増える。

家庭内に置く機材に対して、この費用対効果が見合わないと判断した。

**判断が変わる条件**を書いておく。
機材を宅外に置く、他人が物理的に触れる場所に移す、EliteDesk を中古市場へ売却する、のいずれかである。
売却だけなら `talosctl reset` で足りる。

**EPHEMERAL の暗号化は空のパーティションにしか掛からない。**
後から有効にするとノードの作り直しになるため、入れるならクラスターを組むときに決める。

...
以下の2件は、BGP を疎通させた直後に測ることにしていた。
R4（2026年9月9日）で実測し、どちらも決着した。
測り方と経緯は [bgp-peering.md](bgp-peering.md) にある。

**UniFi の Zone-Based Firewall は、宛先ネットワークでゾーンを決める。**
BGP で学習した `/32` は、next-hop が VLAN 20 にあっても、アドレスの属する VLAN 120 のゾーンに入る。
出力インターフェースでは判定していない。
[../design.md](../design.md) のゾーン間ポリシー表は、この前提のまま成立する。

**VLAN 20 の機器から VLAN 120 の LB IP に届く。**
VLAN 20 に置いた検証端末（`192.168.20.32`）から到達を確認した。
LB Pool 専用の VLAN を切る判断は、狙いどおりに効いている。

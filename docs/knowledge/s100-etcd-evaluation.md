...
測定に使ったスクリプト（`talos/phase1/`）と生の測定結果は削除した。
再実行する筋道がないためである。
スクリプトは git 履歴に残っているので、必要になれば `git log -- talos/phase1` から取り出せる。
以降に出てくる `talos/phase1/...` のコマンドは、当時こう実行したという記録である。
...
代わりに手元の Mac から定期的に curl するスクリプトを置いた（削除済み。「前提の変更」を参照）。
